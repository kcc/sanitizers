...
struct SuperPage {
  ...
  void *TryAllocate(bool DataOnly, SizeClassDescr SCD, size_t *HintPtr) {
    ...
    *HintPtr = Pos + 1; // RoundDownTo(Pos, 32);
    ...
    Res = ApplyAddressTag(Res, GetMemoryTag(Res));
    if (0) {
      fprintf(stderr,
  ...
  uint8_t UpdateMemoryTagOnFree(void *P, size_t Size) {
    if (!Config.UseShadow) return 0;
    uint8_t OldMemoryTag = GetMemoryTag(P);
    uint8_t NewMemoryTag = OldMemoryTag + 1;  // or random?
    SetMemoryTag(P, Size, NewMemoryTag);
    // if (GetMemoryTag(Ptr) != NewMemoryTag) TRAP();
    return NewMemoryTag;
  }
...
struct Allocator {
  ...
  void CountAccess(void *Ptr) {
    if (IsMine(Ptr))
      TLS.Stats.AccessesPerSizeClass[GetSizeClass(reinterpret_cast<uintptr_t>(
                                                      ApplyAddressTag(Ptr, 0)))
                                         .v]++;
    else
  ...
  void *RemoveAddressTagAndCheckForDoubleFree(void *Ptr) {
    uint8_t AddressTag = GetAddressTag(Ptr);
    Ptr = ApplyAddressTag(Ptr, 0);
    uint8_t MemoryTag = GetMemoryTag(Ptr) & 15;
...
//            BytesInQuarantine);
  ...
  SuperPage *AllocateSuperPage(size_t Size) {
    ...
    for (size_t Pos = Res->This(), End = Pos + ChunkSize * SCD.NumChunks;
      ...
      SetMemoryTag(reinterpret_cast<void*>(Pos), ChunkSize, RandR(&TLS.Rand));
    super_pages[SC.v]++;
